...
into the asset's parts at an anchor. This is what makes levels look like games.
...
def _s(off, size, color, material="SmoothPlastic", shape="Block", cc=True, klass="Part"):
    return {"off": off, "size": size, "color": color, "material": material,
            "shape": shape, "cc": cc, "class": klass}
...
ASSETS = {
    ...
    "torch": lambda p: [
        ...
        _s([0, 5.4, 0], [1.1, 1.4, 1.1], "#ff7a1a", "Neon", "Ball", cc=False)],
    "lamp": lambda p: [
        _s([0, 4, 0], [0.6, 8, 0.6], "#2b2f36", "Metal", "Cylinder"),
        _s([0, 8.3, 0], [1.6, 1.6, 1.6], "#fff1a8", "Neon", "Ball", cc=False)],
    "crystal": lambda p: [
        _s([0, 2.4, 0], [2, 5, 2], (p[1] if len(p) > 1 else "#7c4dff"), "Glass", "Cylinder", cc=False),
    ...
    "gem": lambda p: [_s([0, 2, 0], [2.2, 2.6, 2.2], "#00e5ff", "Neon", "Ball", cc=False)],
...
# Which assets read as gameplay objects (single interactive part).
GAMEPLAY_ASSETS = ["coin", "gem", "orb", "button", "flag", "coin_pile"]


def expand(asset: str, folder: str, name: str, anchor, palette) -> list:
    """A placement -> the asset's part ops at the anchor."""
    fn = ASSETS.get(asset, ASSETS["crate"])
    subs = fn(palette or [])
    multi = len(subs) > 1
    ops = []
    for i, s in enumerate(subs):
        pos = [anchor[0] + s["off"][0], anchor[1] + s["off"][1], anchor[2] + s["off"][2]]
        nm = f"{name}_{i}" if multi else name
        ops.append(_mkop(folder, nm, pos, s["size"], s["color"], s["material"],
                         s.get("cc", True), s.get("class", "Part"), s.get("shape", "Block")))
    return ops
